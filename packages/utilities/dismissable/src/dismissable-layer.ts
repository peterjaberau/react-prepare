import { contains, getEventTarget, isHTMLElement, raf } from "@ibrains-design/dom-query"
import {
  trackInteractOutside,
  type FocusOutsideEvent,
  type InteractOutsideHandlers,
  type PointerDownOutsideEvent,
} from "@ibrains-design/interact-outside"
import { isFunction, warn, type MaybeFunction } from "@ibrains-design/utils"
...
function trackDismissableElementImpl(node: MaybeElement, options: DismissableElementOptions) {
  if (!node) {
    warn("[@ibrains-design/dismissable] node is `null` or `undefined`")
    return
  }
...
export function trackDismissableBranch(nodeOrFn: NodeOrFn, options: { defer?: boolean } = {}) {
  ...
  cleanups.push(
    func(() => {
      const node = isFunction(nodeOrFn) ? nodeOrFn() : nodeOrFn
      if (!node) {
        warn("[@ibrains-design/dismissable] branch node is `null` or `undefined`")
        return
      }
