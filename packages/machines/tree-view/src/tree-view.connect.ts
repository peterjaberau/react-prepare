import { getEventKey, isModifierKey, type EventKeyMap } from "@ibrains-design/dom-event"
import { dataAttr, getEventTarget, isComposingEvent, isEditableElement } from "@ibrains-design/dom-query"
import type { NormalizeProps, PropTypes } from "@ibrains-design/types"
import { add, isEqual, remove, uniq } from "@ibrains-design/utils"
    ...
    getTreeProps() {
      return normalize.element({
        ...
        onKeyDown(event) {
          ...
          if (nodeId == null) {
            console.warn(`[ibrainsd-js/tree-view] Node id not found for node`, node)
            return
          }
