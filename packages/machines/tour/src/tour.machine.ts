import { createMachine, guards, ref } from "@ibrains-design/core"
import { trackDismissableBranch } from "@ibrains-design/dismissable"
import { contains, isHTMLElement } from "@ibrains-design/dom-query"
import { trapFocus } from "@ibrains-design/focus-trap"
import { trackInteractOutside } from "@ibrains-design/interact-outside"
import { getPlacement } from "@ibrains-design/popper"
import { compact, isEqual, isString, nextIndex, prevIndex } from "@ibrains-design/utils"
      ...
      actions: {
        ...
        validateSteps(ctx) {
          ...
          ctx.steps.forEach((step) => {
            if (ids.has(step.id)) {
              throw new Error(`[ibrainsd-js/tour] Duplicate step id: ${step.id}`)
            }

            if (step.target == null && step.type == null) {
              throw new Error(`[ibrainsd-js/tour] Step ${step.id} has no target or type. At least one of those is required.`)
            }
