import { ref, snapshot, subscribe } from "@ibrains-design/store"
import {
  cast,
  clear,
  compact,
...
> {
  ...
  public sendChild = (evt: S.Event<S.AnyEventObject>, to: string | ((ctx: TContext) => string)) => {
    ...
    if (!child) {
      invariant(`[@ibrains-design/core] Cannot send '${event.type}' event to unknown child`)
    }
    child!.send(event)
  }
  ...
  public stopChild = (id: string) => {
    if (!this.children.has(id)) {
      invariant(`[@ibrains-design/core > stop-child] Cannot stop unknown child ${id}`)
    }
  ...
  private getDelayedEventActions = (state: TState["value"]) => {
    ...
    if (isArray(stateNode.after)) {
      ...
      if (!hasProp(transition, "delay")) {
        throw new Error(`[@ibrains-design/core > after] Delay is required for after transition: ${JSON.stringify(transition)}`)
      }
  ...
  private executeActions = (actions: S.Actions<TContext, TState, TEvent> | undefined, event: TEvent) => {
    ...
    for (const action of toArray(pickedActions)) {
      ...
      warn(
        isString(action) && !fn,
        `[@ibrains-design/core > execute-actions] No implementation found for action: \`${action}\``,
      )
  ...
  private executeActivities = (
    ...
    for (const activity of activities) {
      ...
      if (!fn) {
        warn(`[@ibrains-design/core > execute-activity] No implementation found for activity: \`${activity}\``)
        continue
      }
  ...
  public sendParent = (evt: S.Event<S.AnyEventObject>) => {
    if (!this.parent) {
      invariant("[@ibrains-design/core > send-parent] Cannot send event to an unknown parent")
    }
  ...
  public transition = (state: TState["value"] | S.StateInfo<TContext, TState, TEvent> | null, evt: S.Event<TEvent>) => {
    ...
    if (!stateNode && !this.config.on) {
      const msg =
        this.status === MachineStatus.Stopped
          ? "[@ibrains-design/core > transition] Cannot transition a stopped machine"
          : `[@ibrains-design/core > transition] State does not have a definition for \`state\`: ${state}, \`event\`: ${event.type}`
      warn(msg)
      return
    }
