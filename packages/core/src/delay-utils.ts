import { invariant, isFunction, isNumber, isString } from "@ibrains-design/utils"
  ...
  return (context: TContext, event: TEvent) => {
    ...
    if (isString(delay)) {
      ...
      if (delaysMap) {
        ...
        invariant(
          valueOrFn == null,
          `[@ibrains-design/core > determine-delay] Cannot determine delay for \`${delay}\`. It doesn't exist in \`options.delays\``,
        )
