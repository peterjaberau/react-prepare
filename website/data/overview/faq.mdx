...
## How do I upgrade all ibrainsd dependencies?

Since we use independent versioning for each ibrainsd package, it can sometimes be
inconvenient to upgrade all dependencies individually.

You can use scoped upgrades feature in your package manager to update all ibrainsd
packages seamlessly.

```bash
pnpm up "@ibrains-design/*"
# or
yarn upgrade --scope @ibrains-design
# or
npm up @ibrains-design/...
```

## Why am I getting `proxyState is not iterable` error?

This happens when there's a mismatch in the versions of `@ibrains-design/*` modules or
multiple instances of `@ibrains-design/store` are installed. It is common with PNPM
monorepo users.

To fix this, you can use the `overrides` feature in your `package.json` to
ensure that all `@ibrains-design/*` modules are on the same version.

```json
{
  "pnpm": {
    "overrides": {
      "@ibrains-design/store": "^0.12.0"
    }
  }
}
```
