...
import { EditorStoreType, PopOverFormContext, SubEditorContext } from "./store/editor"
...
const builtInPlugins: Array<PluginClass | [PluginClass, Record<string, any> | (() => Record<string, any>)]> = []
  ...
  if (klass.priority == null || !Number.isInteger(klass.priority)) {
    ...
    exsitedPluginIdx = ~exsitedPluginIdx
      ...
      : builtInPlugins.findIndex(
          (item) => !Array.isArray(item) && item.id === klass.id && item?.prototype instanceof BasePlugin,
        )
...
export function getEditorPlugins(options: any = {}) {
  ...
  return builtInPlugins.filter((item) => (Array.isArray(item) ? item[0] : item).scene?.includes(scene))
}

export function unRegisterEditorPlugin(id: string) {
  const idx = findIndex(builtInPlugins, (item) => (Array.isArray(item) ? item[0].id === id : item.id === id))
...
export class EditorManager {
  ...
  constructor(
    ...
    this.env = {
      ...env, //The default env contains jumpTo
      ...omit(config.amisEnv, "replaceText"), // Users can also set custom environment configurations to override the default env
      theme: config.theme,
    }
