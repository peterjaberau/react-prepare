...
export default observer(function ({
  ...
  const handleWResizerMouseDown = React.useCallback((e: MouseEvent) => startResize(e, "horizontal"), [])

  const handleHResizerMouseDown = React.useCallback((e: MouseEvent) => startResize(e, "vertical"), [])

  const handleResizerMouseDown = React.useCallback((e: MouseEvent) => startResize(e, "both"), [])

  const startResize = React.useCallback((e: MouseEvent, direction: "horizontal" | "vertical" | "both") => {
  ...
  const wResizerRef = React.useCallback((ref: HTMLElement) => {
    if (ref) {
      ...
      wResizerDom.current?.removeEventListener("mousedown", handleWResizerMouseDown)
    }
  ...
  const hResizerRef = React.useCallback((ref: HTMLElement) => {
    if (ref) {
      ...
      hResizerDom.current?.removeEventListener("mousedown", handleHResizerMouseDown)
    }
  ...
  const resizerRef = React.useCallback((ref: HTMLElement) => {
    if (ref) {
      ...
      resizerDom.current?.removeEventListener("mousedown", handleResizerMouseDown)
    }
  ...
  const isHover = store.isHoved(id) || store.dropId === id || store.insertOrigId === id || curFreeContainerId === id
  const isDraggableContainer = store.draggableContainer(id)

  const aePreviewOffsetWidth = document.getElementById("aePreviewHighlightBox")!.offsetWidth

  if (!isAlive(node)) {
    return <div />
  }

  const isRightElem = aePreviewOffsetWidth - node.x < 176
  ...
  return (
    <div
      className={cx(
        "ae-Editor-hlbox",
        {
          ...
          hover: isHover,
          regionOn: node.childRegions.some((region) => store.isRegionHighlighted(region.id, region.region)),
      ...
      data-hlbox-id={id}
      style={{
        display: node.w && node.h && !node.info.plugin.notHighlight ? "block" : "none",
        top: node.y,
        left: node.x,
        width: node.w,
        height: node.h,
    ...
    >
      {isActive && !readonly ? (
        <div className={`ae-Editor-toolbarPopover ${isRightElem ? "is-right-elem" : ""}`}>
          <div className="ae-Editor-nav">
            {node.host ? (
              <div className="ae-Editor-tip parent" onClick={() => onSwitch?.(node.host.id)}>
                {node.host.label}
              </div>
            ) : null}
            ...
            {node.firstChild ? (
              <div className="ae-Editor-tip child" onClick={() => onSwitch?.(node.firstChild.id)}>
      ...
      {node.widthMutable && node.heightMutable ? <span className="ae-Resizer" ref={resizerRef}></span> : null}
    </div>
  )
})
