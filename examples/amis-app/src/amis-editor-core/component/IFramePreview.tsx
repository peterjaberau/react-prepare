...
import { autobind, closeContextMenus, findTree, render, resizeSensor } from "amis"
...
export default class IFramePreview extends React.Component<IFramePreviewProps> {
  ...
  constructor(props: IFramePreviewProps) {
    ...
    const styles = [].slice.call(document.querySelectorAll('link[rel="stylesheet"], style')).map((el: any) => {
      return el.outerHTML
    })
  ...
  componentDidMount() {
    if (this.props.autoFocus) {
      ...
      setTimeout(() => {
        if (isAlive(store)) {
          const first = findTree(store.outline, (item) => !item.isRegion && item.clickable)
  ...
  render() {
    const { editable, store, appLocale, autoFocus, env, data, manager, ...rest } = this.props
