...
export class SubEditor extends React.Component<SubEditorProps> {
  constructor(props: SubEditorProps) {
    super(props)
    this.afterResolveEditorInfo = this.afterResolveEditorInfo.bind(this)
    this.handleBuildPanels = this.handleBuildPanels.bind(this)
  }
  ...
  afterResolveEditorInfo(event: PluginEvent<RendererInfoResolveEventContext>) {
    ...
    if (!slot) {
      if (context.data && !context.schemaPath && store.subEditorContext?.memberImmutable) {
    ...
    if (!~context.schemaPath.indexOf(slotPath) && context.data) {
      ...
      context.data.memberImmutable = !Array.isArray(store.subEditorContext?.value)
